  ...
  >
    ...
    <div class="col-6 row no-wrap" @click="increment()">
      ...
      <q-img class="col-8"
          ...
          :src="getCardImage(this.card.asset)"
          :alt="card.name"
          
      />
      <div class="col-4">
        <span :class="'card-list-' + card.type">{{card.type}}</span><br />
        <ManySymbols v-bind:resources="card.resources" /> <br />
...
<script>
import BlockData from "components/cards/detail/BlockData.vue";
import AttackData from "components/cards/detail/AttackData.vue";
...
export default {
  ...
  setup(props) {
    ...
    function increment() {
      $store.increment(props.card);
    }

    function decrement() {
      $store.decrement(props.card);
    }

    const myQty = computed(() => $store.quantity(props.card.asset))

    const amMain = computed(() => $store.getFace?.name == props.card["name"])

    return {
        increment, decrement, isCharacter, myQty, amMain, getCardImage
    }
  },
  props: {
    card: Object,
